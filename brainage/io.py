...
def load_best_params(json_file="best_params.json"):
    ...
    try:
        ...
        if "outlier_method_name" in best_params:
            ...
            if (
                outlier_method == OutlierDetector.isoforest
                and "isoforest_contamination" in best_params
            ):
                configs["isoforest_contamination"] = best_params[
                    "isoforest_contamination"
                ]
            elif (
                outlier_method == OutlierDetector.zscore and "zscore_std" in best_params
            ):
        ...
        if "selection_percentile" in best_params:
            ...
            print(
                f"   Loaded selection_percentile: {best_params['selection_percentile']}"
            )

        if "pca_isoforest_contamination" in best_params:
            configs["pca_isoforest_contamination"] = best_params[
                "pca_isoforest_contamination"
            ]
            print(
                f"   Loaded pca_isoforest_contamination: {best_params['pca_isoforest_contamination']}"
            )
...
def save_results_locally(results_df, is_grouped_run):
    ...
    if is_grouped_run:
        summary_stats = results_df.groupby(["model", "outlier_method"])[
            "validation_score"
        ].describe()
    else:
