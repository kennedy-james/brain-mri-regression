...
def build_nn(X):
    ...
    nn_model = TransformedTargetRegressor(
        regressor=nn_pipe, transformer=StandardScaler()
    )
    return nn_model
...
def fit(X, y):
    ...
    if model_name is Regressor.stacking:
        ...
        xgb_pipeline = Pipeline([("feature_selection", xgb_f), ("xgb", xgb_model)])
        #################################
        ...
        svr_model = SVR(C=86, epsilon=0.11)
        ...
        nn_pipeline = Pipeline([("feature_selection", nn_f), ("nn", nn_model)])
        ...
        for i in range(configs["nn_depth"] + 1):
            ...
            network_architecture.append(
                nn.Linear(configs["nn_width"][i], configs["nn_width"][i + 1])
            )
...
def run_cv_experiment(x_data, y_data):
    ...
    folds = KFold(
        n_splits=configs["folds"], shuffle=True, random_state=configs["random_state"]
    )
    ...
    for i, (train_index, validation_index) in enumerate(folds.split(x_data)):
        ...
        print(
            f"Fold {i}: Train R² = {train_score:.4f}, Validation R² = {val_score:.4f}"
        )
