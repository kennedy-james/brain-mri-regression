...
def outlier_detection(X, y):
    ...
    def safe_detector(predict_fn):
        def wrapped(X_data):
            ...
            if num_inliers == 0:
                print(
                    f"WARNING: {method} removed all samples. Keeping all as fallback."
                )
                return np.ones(X_data.shape[0], dtype=bool)

            if X_data.shape[0] == X.shape[0]:
                print(
                    f"INFO: {method} (on train data) found {num_outliers} outliers. Keeping {num_inliers} / {num_total} samples."
                )
            return detector
    ...
    if method is OutlierDetector.zscore:
        ...
        get_detector = safe_detector(
            lambda X_data: clf.predict(X_data) == 0
        )  # inliers are labeled 0, outliers 1

    elif method is OutlierDetector.isoforest:
        iso = IsolationForest(
            contamination=configs["isoforest_contamination"],
            random_state=configs["random_state"],
        )
        iso.fit(X)
        print(f"Using IsolationForest (stateful, contamination={iso.contamination})")
        get_detector = safe_detector(
            lambda X_data: iso.predict(X_data) == 1
        )  # inliers are labeled 1, outliers -1
        ...
        get_detector = safe_detector(
            lambda X_data: clf.predict(X_data) == 1
        ...
        pca_svm_pipeline = make_pipeline(
            StandardScaler(),
            PCA(
                n_components=configs["pca_n_components"],
                random_state=configs["random_state"],
            ),
            OneClassSVM(
                nu=configs["pca_svm_nu"], kernel="rbf", gamma=configs["pca_svm_gamma"]
            ),  # rbf is fast on low-dim data
        )

        print(
            f"Using PCA+SVM detector (stateful, n_components={n_components_pca}, nu={configs['pca_svm_nu']}, gamma={configs['pca_svm_gamma']})"
        )
        pca_svm_pipeline.fit(X)
        get_detector = safe_detector(
            lambda X_data: pca_svm_pipeline.predict(X_data) == 1
        )  # inliers 1, outliers -1
        ...
        pca_isoforest_pipeline = make_pipeline(
            ...
            IsolationForest(
                contamination=configs["pca_isoforest_contamination"],
                random_state=configs["random_state"],
            ),
        )
        print(
            f"Using PCA+IsolationForest detector (stateful, n_components={configs['pca_n_components']}, contamination={configs['pca_isoforest_contamination']})"
        )
        pca_isoforest_pipeline.fit(X)
        get_detector = safe_detector(
            lambda X_data: pca_isoforest_pipeline.predict(X_data) == 1
        )  # inliers 1, outliers -1
